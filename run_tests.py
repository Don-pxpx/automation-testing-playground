...
from config.settings import TEST_TARGETS_CONFIG
        ...
        if test_type == "api":
            ...
            if target:
                result = subprocess.run([
                    ...
                    "-f", "performance/load_testing/locustfile.py",
                    "--host", target,
                    "--users", "10",
                    "--spawn-rate", "2",
                    "--run-time", "60s",
...
def run_security_tests(test_type: str = "all", target: Optional[str] = None):
    ...
    try:
        if test_type == "scan":
            ...
            from security.vulnerability_scan.security_scanner import run_security_scan
            
            if target:
                results = run_security_scan(target)
            else:
                logger.info("No target specified, scanning all configured targets")
                from security.vulnerability_scan.security_scanner import scan_all_targets
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python run_tests.py performance api                    # Run API performance tests
  python run_tests.py performance load --target https://example.com  # Run load test
  python run_tests.py security scan --target https://example.com     # Run security scan
  python run_tests.py security auth                     # Run authentication tests
  python run_tests.py integration                       # Run integration tests
  python run_tests.py locust --target https://example.com --users 50  # Run Locust test
  python run_tests.py targets                           # List available targets
        """
    )
