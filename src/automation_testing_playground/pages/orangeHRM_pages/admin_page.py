...
class AdminPage:
    def __init__(self, page: Page):
        ...
        # Admin menu locators
        self.job_menu = "a:has-text('Job')"
        self.job_titles_menu = "a:has-text('Job Titles')"
        self.job_categories_menu = "a:has-text('Job Categories')"
        self.pay_grades_menu = "a:has-text('Pay Grades')"
        self.employment_status_menu = "a:has-text('Employment Status')"
        
        # Job Titles page locators
        self.add_job_title_button = "button:has-text('Add')"
        ...
        self.cancel_job_title_button = "button:has-text('Cancel')"
        
        # Job Titles list locators
        self.job_titles_table = "div.oxd-table-body"
        self.job_title_row = "div.oxd-table-row"
        self.edit_job_title_button = "button:has-text('Edit')"
        self.delete_job_title_button = "button:has-text('Delete')"
    ...
    def click_add_job_title(self):
        ...
        self.page.wait_for_selector(self.add_job_title_button, timeout=10000)
        self.page.click(self.add_job_title_button)
        self.page.wait_for_timeout(2000)  # Wait for form to load
        # Try different field locators
        try:
            self.page.wait_for_selector(self.job_title_name_field, timeout=5000)
        except:
            # Try alternative locators
            try:
                self.page.wait_for_selector("input[placeholder*='Job Title']", timeout=5000)
            except:
                self.page.wait_for_selector("input", timeout=5000)  # Any input field
        self.logger.success("Add Job Title form loaded")

    def fill_job_title_form(self, job_title: str, description: str = "", note: str = ""):
        """Fill the job title form with provided data"""
        self.logger.step(f"Fill job title form: {job_title}")
        
        # Fill job title name
        self.page.wait_for_selector(self.job_title_name_field, timeout=10000)
        self.page.fill(self.job_title_name_field, job_title)
        
        # Fill description if provided
        if description:
            self.page.fill(self.job_description_field, description)
        
        # Fill note if provided
        if note:
            self.page.fill(self.note_field, note)
        
        self.logger.success("Job title form filled successfully")

    def fill_job_title_form_with_model(self, job_title_data: JobTitleData):
        """Fill job title form using Pydantic model."""
        self.fill_job_title_form(
            job_title_data.job_title,
            job_title_data.job_description or "",
            job_title_data.note or ""
        )

    def save_job_title(self):
        """Save the job title form"""
        self.logger.step("Save job title")
        self.page.wait_for_selector(self.save_job_title_button, timeout=10000)
        self.page.click(self.save_job_title_button)
    ...
    def cancel_job_title(self):
        ...
        self.page.wait_for_selector(self.cancel_job_title_button, timeout=10000)
        self.page.click(self.cancel_job_title_button)
        self.logger.success("Job title creation cancelled")

    def search_job_title(self, job_title: str):
    ...
    def edit_job_title(self, job_title: str, new_title: str, new_description: str = ""):
        ...
        self.page.wait_for_selector(self.edit_job_title_button, timeout=10000)
        self.page.click(self.edit_job_title_button)
        
        # Wait for edit form to load
        self.page.wait_for_selector(self.job_title_name_field, timeout=10000)
        
        # Clear and fill the form
        self.page.fill(self.job_title_name_field, "")  # Clear first
        self.page.fill(self.job_title_name_field, new_title)
        
        if new_description:
            self.page.fill(self.job_description_field, "")  # Clear first
            self.page.fill(self.job_description_field, new_description)
    ...
    def delete_job_title(self, job_title: str):
        """Delete a job title"""
        self.logger.step(f"Delete job title: {job_title}")
        
        # Find and click delete button for the specific job title
        self.page.wait_for_selector(self.delete_job_title_button, timeout=10000)
        self.page.click(self.delete_job_title_button)
        
        # Confirm deletion
        self.page.wait_for_selector(self.confirm_delete_button, timeout=10000)
        self.page.click(self.confirm_delete_button)
        self.page.wait_for_timeout(2000)
        self.logger.success(f"Job title deleted: {job_title}")

    def verify_job_title_exists(self, job_title: str) -> bool:
        ...
        try:
            ...
            table_content = self.page.locator(self.job_titles_table).text_content() or ""
    ...
    def get_job_titles_count(self) -> int:
        """Get the number of job titles in the list"""
        self.logger.step("Get job titles count")
        try:
            rows = self.page.locator(self.job_title_row).all()
            count = len(rows)
